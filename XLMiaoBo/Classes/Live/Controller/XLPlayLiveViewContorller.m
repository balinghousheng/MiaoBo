...
- (UIView *)livingPreView {
    if (_livingPreView == nil) {
        ...
        // livingPreView.autoresizingMask = UIViewAutoresizingFlexibleWidth |
...
- (LFLiveSession *)session {
    if (_session == nil) {
        ...
        /*
         ...
         videoConfiguration:videoConfiguration liveType:LFLiveRTMP];
         */
...
- (IBAction)close {
    ...
    if (self.session.state == LFLivePending ||
        self.session.state == LFLiveStart) {
...
- (IBAction)beginLive {
    ...
    if (self.livingBtn.selected) { // 开始直播
        ...
        self.statusLabel.text =
            [NSString stringWithFormat:@"状态: 直播被关闭\nRTMP: %@",
                                       self.rtmpUrl];
    }
}
...
- (BOOL)beginClick {
    ...
    if (![UIImagePickerController
        ...
        [MBProgressHUD showAlertMessage:
                           @"您的设备没有摄像头或者相关的驱动, "
                           @"不能进行直播"];
        return NO;
    }
...
//{
//    [self.videoCamera removeAllTargets];
//    ALinGPUBeautifyFilter *beautifyFilter = [[ALinGPUBeautifyFilter alloc]
//    init];
