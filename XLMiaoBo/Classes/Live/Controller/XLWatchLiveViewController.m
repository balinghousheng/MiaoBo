...
- (instancetype)init {
    if (self == [super init]) {
        ...
        NSSafeObject *safeObj =
            [[NSSafeObject alloc] initWithObject:self
...
- (XLBottomToolView *)bottomToolView {
    if (_bottomToolView == nil) {
            ...
            UIAlertAction *action = [UIAlertAction
                actionWithTitle:@"确定"
                        ...
                        handler:^(UIAlertAction *_Nonnull action) {
                            ...
                            weakSekf.hidden ? [_renderer start]
                                            : [_renderer stop];
                        }];
        ...
        [self.view insertSubview:bottomToolView
                    aboveSubview:self.placeHolderView];
...
- (UIImageView *)otherView {
    if (_otherView == nil) {
        UIImageView *imageView = [[UIImageView alloc]
            initWithImage:[UIImage imageNamed:@"private_icon_70x70"]];
        imageView.userInteractionEnabled = YES;
        [imageView
            addGestureRecognizer:[[UITapGestureRecognizer alloc]
                                     initWithTarget:self
                                             action:@selector(clickOther)]];
...
- (CAEmitterLayer *)emitterLayer {
    if (!_emitterLayer) {
        ...
        emitterLayer.emitterPosition =
            CGPointMake(self.moviePlayer.view.frame.size.width - 50,
                        self.moviePlayer.view.frame.size.height - 50);
        // 发射器的尺寸大小
        ...
        for (int i = 0; i < 10; i++) {
            ...
            UIImage *image = [UIImage
                imageNamed:[NSString stringWithFormat:@"good%d_30x30", i]];
            // 粒子显示的内容
    ...
    if (_moviePlayer) {
        if (_moviePlayer) {
            [self.view insertSubview:self.placeHolderView
                        aboveSubview:self.moviePlayer.view];
        }
        if (_catEarView) {
                   ...
                   completed:^(UIImage *image, NSData *data, NSError *error,
                       ...
                       dispatch_async(dispatch_get_main_queue(), ^{
                           ...
                           if (image) {
                               [weakSelf
                                   showGifLoding:nil
                                          inView:weakSelf.placeHolderView];
                               weakSelf.placeHolderView.image =
...
- (void)didFinish {
    ...
    __weak typeof(self) weakSelf = self;
    [XLLiveTool GetWithURL:self.hotModel.flv
        success:^(id result) {
...
- (void)autoSendBarrage {
    ...
    if (spriteNumber <= 50) { // 限制屏幕上的弹幕量
        [_renderer receive:[self walkTextSpriteDescriptorWithDirection:
...
long _index = 0;
/// 生成精灵描述 - 过场文字弹幕
- (BarrageDescriptor *)walkTextSpriteDescriptorWithDirection:
        (NSInteger)direction {
    ...
    descriptor.params[@"textColor"] =
        XLColor(arc4random_uniform(256), arc4random_uniform(256),
...
- (void)setAllModels:(NSArray *)allModels {
    ...
    self.catEarView.hotModel =
        allModels[arc4random_uniform((int) allModels.count)];
}
...
- (void)shareMethod {
    ...
    NSString *text =
        [NSString stringWithFormat:@"  主播昵称：%@/n  主播房间号：%@/n  "
                                   @"个性签名：%@/n  在线粉丝数：%@",
                                   _hotModel.myname, _hotModel.roomid,
                                   _hotModel.signatures, _hotModel.allnum];
