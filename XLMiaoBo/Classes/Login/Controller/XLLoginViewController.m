...
#define XLREQUEST                                                              \
    ...
    @"authorize?client_id=2039393085&redirect_uri=http://www.code4app.com/"    \
    @"space-uid-843201.html"
...
- (XLThirdLoginView *)thirdView {
    if (_thirdView == nil) {
        ...
        XLThirdLoginView *thirdView = [[XLThirdLoginView alloc]
            initWithFrame:CGRectMake(0, 0, XLScreenW, 60)];
...
- (IJKFFMoviePlayerController *)player {
    if (_player == nil) {
        ...
        _player = [[IJKFFMoviePlayerController alloc]
            initWithContentURLString:[[NSBundle mainBundle]
...
- (void)sinaMethod {
    ...
    sina.navigationItem.leftBarButtonItem =
        [UIBarButtonItem barButtonItemWithImage:@"back_9x16"
                                 highlightImage:nil
                                         targer:self
    ...
    XLNavigationController *nav =
        [[XLNavigationController alloc] initWithRootViewController:sina];
    ...
    snsPlatform.loginClickHandler(
        ...
        ^(UMSocialResponseEntity *response) {
            ...
            if (response.responseCode == UMSResponseCodeSuccess) {
                ...
                //            valueForKey:UMShareToWechatTimeline];
                //            NSLog(@"username is %@, uid is %@, token is %@ url
                //            is
                ...
                [[UMSocialDataService defaultDataService]
                    requestSnsInformation:UMShareToSina
...
- (void)qq {
    UMSocialSnsPlatform *snsPlatform =
        [UMSocialSnsPlatformManager getSocialPlatformWithName:UMShareToQQ];
    ...
    snsPlatform.loginClickHandler(
        ...
        ^(UMSocialResponseEntity *response) {
            ...
            if (response.responseCode == UMSResponseCodeSuccess) {
                //            UMSocialAccountEntity *snsAccount =
                //            [[UMSocialAccountManager socialAccountDictionary]
                //            valueForKey:UMShareToQQ];
                //            NSLog(@"username is %@, uid is %@, token is %@ url
                //            is
                //            %@",snsAccount.userName,snsAccount.usid,snsAccount.accessToken,snsAccount.iconURL);
                //获取accestoken以及QQ用户信息，得到的数据在回调Block对象形参respone的data属性
                [[UMSocialDataService defaultDataService]
                    requestSnsInformation:UMShareToQQ
                               completion:^(UMSocialResponseEntity *response) {
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    [MBProgressHUD hideHUDForView:self.sina.view];
    [MBProgressHUD showError:@"加载失败，请检查网络是否通畅"
                      toView:self.sina.view];
}
...
- (BOOL)webView:(UIWebView *)webView
    ...
    if (range.length > 0) {
        NSString *requestToken =
            [urlStr substringFromIndex:range.location + range.length];
        // 4.获得访问标记
...
- (void)getAccessToken:(NSString *)requestToken {
    ...
    NSString *Str = [NSString
        stringWithFormat:@"client_id=2039393085&client_secret="
                         @"7da7d4501f3f51a14040e1fa5eca7723&grant_type="
                         @"authorization_code&redirect_uri=http://"
                         @"www.code4app.com/space-uid-843201.html&code=%@",
                         requestToken];
...
- (void)jump {
    ...
    dispatch_after(
        ...
        dispatch_get_main_queue(), ^{
            ...
            [self presentViewController:tab
                               animated:YES
                             completion:^{
