...
- (UITableView *)tableView {
    if (_tableView == nil) {
        UITableView *tableView = [[UITableView alloc]
            initWithFrame:CGRectMake(0, CGRectGetMaxY(self.headerView.frame),
                                     XLScreenW,
                                     XLScreenH - self.headerView.height)];
...
- (XLHotHeaderView *)headerView {
    if (_headerView == nil) {
        XLHotHeaderView *headerView = [[XLHotHeaderView alloc]
            initWithFrame:CGRectMake(0, 0, XLScreenW, 100)];
        [headerView setImageClickBlock:^(XLHotHeaderModel *headerModel) {
            XLCrownRankViewController *web =
...
- (NSArray *)imageArray {
    if (_imageArray == nil) {
        _imageArray = @[
            [UIImage imageNamed:@"reflesh1_60x55"],
...
- (void)loadNewData {
    __weak typeof(self) weakSelf = self;
    [XLLiveTool GetWithHotURL:self.currentPage
        success:^(XLNewResult *result) {
