...
+ (void)initialize {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(
        ...
        stringByAppendingPathComponent:@"XRCarousel"];
    BOOL isDir    = NO;
    BOOL isExists = [[NSFileManager defaultManager] fileExistsAtPath:cache
                                                         isDirectory:&isDir];
    if (!isExists || !isDir) {
    ...
    if (!describeArray.count) {
        ...
        if (describeArray.count < _images.count) {
            NSMutableArray *describes =
                [NSMutableArray arrayWithArray:describeArray];
...
- (void)setScrollViewContentSize {
    if (_images.count > 1) {
        ...
        self.currImageView.frame =
            CGRectMake(self.width * 2, 0, self.width, self.height);
...
- (void)setDescribeTextColor:(UIColor *)color
                        font:(UIFont *)font
...
- (void)setPageImage:(UIImage *)image
    andCurrentPageImage:(UIImage *)currentImage {
...
- (void)setPageColor:(UIColor *)color
    andCurrentPageColor:(UIColor *)currentColor {
...
- (void)setPagePosition:(PageControlPosition)pagePosition {
    ...
    _pageControl.hidden =
        (_pagePosition == PositionHide) || (_imageArray.count == 1);
    ...
    if (!_pageImageSize.width) { //没有设置图片，系统原有样式
        ...
        size = CGSizeMake(_pageImageSize.width *
                              (_pageControl.numberOfPages * 2 - 1),
                          _pageImageSize.height);
    }
    _pageControl.frame = CGRectMake(0, 0, size.width, size.height);

    CGFloat centerY = self.height - size.height * 0.5 - VERMARGIN -
                      (_describeLabel.hidden ? 0 : DES_LABEL_H);
    ...
    if (_pagePosition == PositionNone || _pagePosition == PositionBottomCenter)
        ...
        _pageControl.center =
            CGPointMake(self.width * 0.5, size.height * 0.5 + VERMARGIN);
    else if (_pagePosition == PositionBottomLeft)
        _pageControl.frame =
            CGRectMake(HORMARGIN, pointY, size.width, size.height);
    else
        _pageControl.frame = CGRectMake(self.width - HORMARGIN - size.width,
                                        pointY, size.width, size.height);
}
...
- (void)nextPage {
    if (_changeMode == ChangeModeFade) {
        ...
        [self.scrollView setContentOffset:CGPointMake(self.width * 3, 0)
                                 animated:YES];
}
...
- (void)layoutSubviews {
    ...
    _describeLabel.frame =
        CGRectMake(0, self.height - DES_LABEL_H, self.width, DES_LABEL_H);
    //重新计算pageControl的位置
    self.pagePosition = self.pagePosition;
...
- (void)imageClick {
    ...
    } else if ([_delegate respondsToSelector:@selector(carouselView:
                                                  clickImageAtIndex:)]) {
...
- (void)downloadImages:(int)index {
    ...
    NSBlockOperation *download = [NSBlockOperation blockOperationWithBlock:^{
        ...
        if (image) {
            ...
            if (_currIndex == index)
                [_currImageView performSelectorOnMainThread:@selector(setImage:)
                                                 withObject:image
                                              waitUntilDone:NO];
...
- (void)clearDiskCache {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(
        ...
        stringByAppendingPathComponent:@"XRCarousel"];
    NSArray *contents =
        [[NSFileManager defaultManager] contentsOfDirectoryAtPath:cache
                                                            error:NULL];
    for (NSString *fileName in contents) {
        [[NSFileManager defaultManager]
            removeItemAtPath:[cache stringByAppendingPathComponent:fileName]
                       error:nil];
    }
}
    ...
    if (offsetX < self.width * 2) {
        if (_changeMode == ChangeModeFade) {
            ...
            self.otherImageView.frame =
                CGRectMake(self.width, 0, self.width, self.height);
