...
+ (instancetype)tableViewCell {
    return [[[NSBundle mainBundle] loadNibNamed:@"XLHotTableViewCell"
                                          owner:nil
...
- (void)setHotModel:(XLHotModel *)hotModel {
    ...
    [self.headImageView
        sd_setImageWithURL:[NSURL URLWithString:hotModel.smallpic]
          placeholderImage:[UIImage imageNamed:@"placeholder_head"]
                   options:SDWebImageRefreshCached
                 completed:^(UIImage *image, NSError *error,
                             SDImageCacheType cacheType, NSURL *imageURL) {

                     weakSelf.headImageView.image =
                         [UIImage circleImage:image
    ...
    [self.bigPicView
        sd_setImageWithURL:[NSURL URLWithString:hotModel.bigpic]
    ...
    // 设置当前观众数量
    NSString *fullfans =
        [NSString stringWithFormat:@"%@人在看", hotModel.allnum];
    NSRange range = [fullfans
        rangeOfString:[NSString stringWithFormat:@"%@", hotModel.allnum]];
    NSMutableAttributedString *attr =
        [[NSMutableAttributedString alloc] initWithString:fullfans];
...
- (IBAction)share {
    ...
    NSString *text =
        [NSString stringWithFormat:@"  主播昵称：%@\n  主播房间号：%@\n  "
                                   @"个性签名：%@\n  在线粉丝数：%@",
...
- (void)headerImage:(UIGestureRecognizer *)ges {
    ...
    [userInfoView setSelectedBlock:^{
        ...
        XLWatchLiveViewController *watch =
            [[XLWatchLiveViewController alloc] init];

        watch.hotModel  = _hotModel;
        watch.allModels = weakSelf.allModels;

        [weakSelf.parentVC presentViewController:watch
                                        animated:YES
                                      completion:nil];
    }];
}
