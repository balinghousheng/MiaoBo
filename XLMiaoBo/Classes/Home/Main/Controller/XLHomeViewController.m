...
- (XLCustomTitleView *)titleview {
    if (_titleview == nil) {
        ...
        XLCustomTitleView *titleView = [[XLCustomTitleView alloc]
            initWithFrame:CGRectMake(0, 0, XLScreenW * 0.5, 40)];
...
- (void)setupBasic {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]
        initWithImage:[UIImage imageNamed:@"search_15x14"]
                style:UIBarButtonItemStyleDone
               target:self
               action:@selector(search)];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]
        initWithImage:[UIImage imageNamed:@"head_crown_24x24"]
                style:UIBarButtonItemStyleDone
               target:self
               action:@selector(crownRank)];
}
...
- (void)search {
    ...
    UISearchBar *searchBar = [[UISearchBar alloc]
        initWithFrame:CGRectMake(40, 60, self.view.width - 80, 30)];
...
- (void)crownRank {
    XLCrownRankViewController *crownRank =
        [[XLCrownRankViewController alloc] init];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    self.titleview.underLine.centerX =
        self.scrollView.contentOffset.x / scale + 18;
