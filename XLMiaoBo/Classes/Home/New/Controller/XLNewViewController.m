...
- (void)viewDidLoad {
    ...
    [self.collectionView
                       registerNib:[UINib nibWithNibName:@"XLCollectionViewCell"
                                                  bundle:nil]
        forCellWithReuseIdentifier:reuseID];

    self.timer = [NSTimer scheduledTimerWithTimeInterval:5 * 60
                                                  target:self
                                                selector:@selector(setupBasic)
                                                userInfo:nil
                                                 repeats:YES];
...
- (void)loadNewData {
    [XLLiveTool GetWithNewURL:self.currentPage
        success:^(XLNewResult *result) {
            ...
            if ([result.msg isEqualToString:@"fail"]) {
                ...
                XLNewData *newData =
                    [XLNewData mj_objectWithKeyValues:result.data];
...
- (void)collectionView:(UICollectionView *)collectionView
    ...
    [self.allModels
        enumerateObjectsUsingBlock:^(XLNewModel *newModel, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
